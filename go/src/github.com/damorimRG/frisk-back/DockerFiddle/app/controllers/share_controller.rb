...
class ShareController < ApplicationController
    ...
    ENDPOINT = 'pg.frisk.igorwiese.com'
    ...
    def shared
        s3 = Aws::S3::Resource.new(region: 'us-east-1', 
                           access_key_id: 'AKIAJOVXNQ5CZTWEAZZA', 
                           secret_access_key: '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')
        ...
        session.instances.each do |instance|
            ...
            downloadLink = s3.bucket('dockerfiddle').object("shares/#{session.uuid}/#{instance.uuid}.tar.gz").presigned_url(:get, expires_in: 60 * 60)
    ...
    def share
        ...
        pwdInstances.each do |pwdInstance|
            ...
            Net::SSH.start("direct.#{ENDPOINT}", pwdInstance['proxy_host'], port: 8022) do |ssh|
                ...
                ssh.loop
                url = s3.bucket('dockerfiddle').object("shares/#{session.uuid}/#{instance.uuid}.tar.gz").presigned_url(:put)
