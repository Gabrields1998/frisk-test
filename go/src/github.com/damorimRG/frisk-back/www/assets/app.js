(function() {
  ...
  app.controller('PlayController', ['$scope', '$log', '$http', '$location', '$timeout', '$mdDialog', '$window', 'TerminalService', 'KeyboardShortcutService', 'InstanceService', 'SessionService', 'Upload', function($scope, $log, $http, $location, $timeout, $mdDialog, $window, TerminalService, KeyboardShortcutService, InstanceService, SessionService, Upload) {
    ...
    $scope.buildComposeContainer = function(instance) {
      $scope.socket.emit('instance terminal in', instance.name, 'docker-compose up\n');
    };
    ...
    $scope.stopAllContainers = function(instance) {
      $http({
        method: 'POST',
        url: 'http://frisk.igorwiese.com/killall/',
    ...
    $scope.shareSession = function() {
      $http({
        method: 'POST',
        url: 'http://frisk.igorwiese.com/share/' + $scope.sessionId
      }).then(function(response) {
        $scope.showAlert('Here is your link: http://frisk.igorwiese.com/'+response.data.uuid);
      });
    };
